...
# Function to generate contrast examples for NLI
def generate_contrast_examples(examples, perturbation_fn):
    """
    Create contrast examples by perturbing the hypothesis or premise.
    :param examples: Original dataset.
    :param perturbation_fn: A function that perturbs an example.
    :return: Dataset with contrast examples.
    """
    contrast_premises = []
    contrast_hypotheses = []
    contrast_labels = []
    ...
    for premise, hypothesis, label in zip(examples["premise"], examples["hypothesis"], examples["label"]):
        perturbed_hypothesis = perturbation_fn(hypothesis)
        new_label = determine_new_label(premise, hypothesis, perturbed_hypothesis, label)
        contrast_premises.append(premise)
        contrast_hypotheses.append(perturbed_hypothesis)
        contrast_labels.append(new_label)  # Adjusted label

    return {
        'premise': contrast_premises,
        'hypothesis': contrast_hypotheses,
...
def perturb_example(text):
    """
    A sample perturbation function that modifies input text.
    This can be replaced with task-specific logic for contrast generation.
    """
    words = text.split()
    if not words:
        return text
    ...
    synonyms = get_synonyms(words[random_idx])
    if synonyms:
        synonym = random.choice(synonyms)
        new_words[random_idx] = synonym
    else:
        # If no synonyms found, slightly modify the word
        new_words[random_idx] = words[random_idx][::-1]  # Reverse the word as a simple perturbation
...
def determine_new_label(premise, original_hypothesis, perturbed_hypothesis, original_label):
    """
    Determine the new label after perturbation.
    """
    # This is a heuristic function; you may need to adjust it based on your perturbation logic.
    if "not sure." in perturbed_hypothesis:
        # Hypothesis has become more uncertain, set label to neutral (1)
        return 1  # neutral
    elif "definitely" in perturbed_hypothesis:
        # Hypothesis was negated and "not" was replaced with "definitely"
        # Flip between entailment and contradiction
        if original_label == 0:  # entailment
            return 2  # contradiction
        elif original_label == 2:  # contradiction
            return 0  # entailment
        else:
